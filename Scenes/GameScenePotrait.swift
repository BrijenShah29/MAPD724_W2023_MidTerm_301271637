...
class GameScenePotrait: SKScene
{
    // instance variables
    var ocean1: OceanPotrait?
    var ocean2: OceanPotrait?
    var player: PlayerPotrait?
    var island: IslandPotrait?
    var clouds : [CloudPotrait] = []
    ...
    override func sceneDidLoad()
    {
        ...
        player = PlayerPotrait()
        addChild(player!)
        
        // add the island to the Scene
        island = IslandPotrait()
        addChild(island!)
        
        // add 2 clouds to the Scene
        for _ in 0...1
        {
            let cloud = CloudPotrait()
            clouds.append(cloud)
            addChild(cloud)
        }
